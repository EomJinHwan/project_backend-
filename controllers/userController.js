...
const {insertUser, insertLoginHistory, getUser, formatDate, check, encryptionPw, findUser, getCurrentPw, convertDateFormat, updatePw } = require('../services/query.js');
...
async function FindPassword(req, res) {
    ...
    try{
        ...
        if(user){
            ...
            //조건에 맞는 아이디 있으면 값 반환
            return res.status(200).json({success:true, message : "조건에 맞는 아이디가 있습니다", token});
        }else{
            return res.status(400).json({success:false,message : "조건에 맞는 아이디가 없습니다"})
        }
    } catch (error) {
...
async function ChangePassword(req, res) {
    ...
    console.log(`token : ${token}, newPw : ${newPw}`);
    ...
    try {
        ...
        if(decoded){
            ...
            //기존 비밀번호 조회
            const currentHashedPw = await getCurrentPw(decoded.userId);
            ...
            if(isMatch){
                ...
                //비밀번호 업데이트
                await updatePw(decoded.userId, encryption_pw);
