...
async function CheckUserIdDuplicate(req, res) {
    ...
    try {
         ...
         if (!isDuplicate) {
             // 중복된 아이디가 있을 때
             return res.status(200).json({success:false, message:"as;lkfjklafs중복된 아이디 입니다"});
         } else{
             // 중복되지 않은 경우
...
//             }
//         } else {
//             return res.status(400).json({success:false, message : "현재 비밀번호를 정확하게 입력해주세요"});
//         }
//     } catch (error) {
//         console.error("비밀번호 변경 중 오류 발생:", error); // 서버 로그에 에러 기록
//         return res.status(500).json({success:false, message: "서버 오류가 발생했습니다" });
...
async function RemoveUser(req, res){
    ...
    try {
        ...
        pool.query(deleteUser, userId, (error, results) => {  //connection
            if(error){
                console.error("오류", error);
                return res.status(500).json({success:false, message: "쿼리 실행중 오류"});
            }
            console.log(results);
            console.log("회원탈퇴 성공");
        ...
        res.status(500).json({success:false, message: "서버 오류가 발생했습니다" });
        console.error("회원탈퇴 실행중 오류 발생:", error);
    }
};
